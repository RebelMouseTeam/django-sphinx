...
class SphinxClient:
    ...
    def SetFilter(self, attribute, values, exclude=0):
        ...
        self._filters.append({'type': SPH_FILTER_VALUES, 'attr': attribute,
                              'exclude': exclude, 'values': values})
    ...
    def SetFilterRange(self, attribute, min_, max_, exclude=0):
        ...
        self._filters.append({'type': SPH_FILTER_RANGE, 'attr': attribute,
                              'exclude': exclude, 'min': min_, 'max': max_})
    ...
    def SetFilterFloatRange(self, attribute, min_, max_, exclude=0):
        ...
        assert(min_ <= max_)
        self._filters.append({'type': SPH_FILTER_FLOATRANGE, 'attr': attribute,
                             'exclude': exclude, 'min': min_, 'max': max_})
    ...
    def SetGroupBy(self, attribute, func, groupsort='@group desc'):
        ...
        """
        assert(isinstance(attribute, str))
        assert(func in [SPH_GROUPBY_DAY, SPH_GROUPBY_WEEK, SPH_GROUPBY_MONTH,
               SPH_GROUPBY_YEAR, SPH_GROUPBY_ATTR, SPH_GROUPBY_ATTRPAIR])
    ...
    def AddQuery(self, query, index='*', comment=''):
        ...
        req = []
        req.append(
            pack('>4L', self._offset, self._limit, self._mode, self._ranker))
        ...
        req.append(
            pack('>LLL', self._cutoff, self._retrycount, self._retrydelay))
        ...
        if len(self._anchor) == 0:
            ...
            attrlat, attrlong = self._anchor[
                'attrlat'], self._anchor['attrlong']
    ...
    def RunQueries(self):
        ...
        req = ''.join(self._reqs)
        length = len(req) + 8
        req = pack('>HHLLL', SEARCHD_COMMAND_SEARCH,
                   VER_COMMAND_SEARCH, length, 0, len(self._reqs)) + req
        self._Send(sock, req)
        ...
        for i in range(0, nreqs, 1):
                ...
                for i in range(len(attrs)):
                    if attrs[i][1] == SPH_ATTR_FLOAT:
                        match['attrs'][attrs[i][0]
                                       ] = unpack('>f', response[p:p + 4])[0]
                    elif attrs[i][1] == SPH_ATTR_BIGINT:
                        match['attrs'][attrs[i][0]
                                       ] = unpack('>q', response[p:p + 8])[0]
                        p += 4
                        ...
                        for n in range(0, nvals, 1):
                            match['attrs'][attrs[i][
                                0]].append(unpack('>L', response[p:p + 4])[0])
                            p += 4
                        p -= 4
                        ...
                        for n in range(0, nvals, 1):
                            match['attrs'][attrs[i][
                                0]].append(unpack('>q', response[p:p + 8])[0])
                            p += 8
                        p -= 4
                    else:
                        match['attrs'][attrs[i][0]
            ...
            result['total'], result['total_found'], result[
                'time'], words = unpack('>4L', response[p:p + 16])
    ...
    def BuildExcerpts(self, docs, index, words, opts=None):
        ...
        # add header
        req = pack('>2HL', SEARCHD_COMMAND_EXCERPT,
                   VER_COMMAND_EXCERPT, length) + req
        self._Send(sock, req)
    ...
    def UpdateAttributes(self, index, attrs, values, mva=False):
        ...
        length = len(req)
        req = pack(
            '>2HL', SEARCHD_COMMAND_UPDATE, VER_COMMAND_UPDATE, length) + req
        self._Send(sock, req)
    ...
    def BuildKeywords(self, query, index, hits):
        ...
        req = ''.join(req)
        length = len(req)
        req = pack('>2HL', SEARCHD_COMMAND_KEYWORDS,
                   VER_COMMAND_KEYWORDS, length) + req
        self._Send(sock, req)
    ...
    def FlushAttributes(self):
        ...
        request = pack('>hhI', SEARCHD_COMMAND_FLUSHATTRS,
                       VER_COMMAND_FLUSHATTRS, 0)  # cmd, ver, bodylen
